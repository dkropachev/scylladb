...
SEASTAR_TEST_CASE(test_forwarded_client_state_preserves_tablet_routing_source) {
    return do_with_cql_env_thread([](cql_test_env& e) {
        constexpr unsigned source_shard = 7;
        auto source_host = e.local_db().get_token_metadata().get_my_id();

        e.local_client_state().set_tablet_routing_source(source_host, source_shard);

        service::forwarded_client_state forwarded(e.local_client_state());
        service::client_state restored(
                e.local_auth_service(),
                &e.service_level_controller_service().local(),
                std::move(forwarded));

        BOOST_REQUIRE(restored.get_tablet_routing_source_host().has_value());
        BOOST_REQUIRE_EQUAL(*restored.get_tablet_routing_source_host(), source_host);
        BOOST_REQUIRE(restored.get_tablet_routing_source_shard().has_value());
        BOOST_REQUIRE_EQUAL(*restored.get_tablet_routing_source_shard(), source_shard);
    });
}
